...
def field_category(field_type_cate: str, type_engine: TypeEngine, llm: Optional[LLM] = None,
    ...
    is_unique_pk_cons: whether primary key, foreign key, or unique key（包含jointly forming composite的主键）
    """
    ...
    if field_type_cate == type_engine.field_type_date_label:
        ...
        if is_date_time == 'yes':
            ...
            if field_type_cate == type_engine.field_type_string_label:
                # non-datetime string，determineyescode、text还yesenum
                res = call_llm(
                ...
                # non-datetime numeric，determineyescode、measure还yesenum
...
class SchemaEngine(SQLDatabase):
    ...
    def execute(self, sql_query: str, timeout=10) -> Any:
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            try:
                ...
                print(f"SQLexecution timeout（{timeout}秒）{sql_query}.")
                return None
            except Exception as e:
                print("exception occurred when executing SQL。", e)
                return None
    ...
    def get_single_field_info_str(self, table_name: str, field_name: str)->str:
        """
        Information about a column：column name、类型、column description、yesno主键、max/min values等
        """
        ...
        field_info_str.append(f'primary key status(或者与其他字段组成composite primary key): {primary_key}')
        ...
        if date_min_gran is not None:
            field_info_str.append(f'The semantics of this field may be related to dates or times，estimated minimum time granularityyes: {date_min_gran}')
    ...
    def fields_category(self):
        ...
        for table_name in tables.keys():
            ...
            for field_name, field_info in fields.items():
                ...
                # For enum type fields，get all enum candidate values
    ...
    def table_and_column_desc_generation(self, language: str='CN'):
        ...
        Four modes：
        no_comment: without any description
        origin: 跟database中保持一致
        generation: 清除已有的描述信息，fully generated by model
        merge: generate descriptions for those without；keep existing descriptions
        """
        ...
        """1、初步理解database的基本信息和content of each table"""
        ...
        for table_name, table_info in self._mschema.tables.items():
            ...
            """2、Classify by dimension and measure，understand differences and relationships between dimensions/measures，for reference"""
            supp_info = {}
            ...
            """3、对每一列生成column description"""
            for field_name, field_info in fields.items():
                field_info_str = self.get_single_field_info_str(table_name, field_name)
                dim_or_meas = field_info.get("dim_or_meas", '')
                field_desc = field_info.get('comment', '')
                if len(field_desc) == 0:  # 原来没有field description，regenerate
            ...
            """4、Table description generation"""
            table_mschema = self._mschema.single_table_mschema(table_name)
